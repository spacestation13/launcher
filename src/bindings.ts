...
async hubSteamLogin(acceptedTos: boolean, preferredUsername: string | null) : Promise<Result<AuthState, CommandError>> {
    try {
    return { status: "ok", data: await TAURI_INVOKE("hub_steam_login", { acceptedTos, preferredUsername }) };
} catch (e) {
    if(e instanceof Error) throw e;
    else return { status: "error", error: e  as any };
}
...
async steamAuthenticate(createAccountIfMissing: boolean, acceptedTos: boolean, preferredUsername: string | null) : Promise<Result<SteamAuthResult, CommandError>> {
    try {
    return { status: "ok", data: await TAURI_INVOKE("steam_authenticate", { createAccountIfMissing, acceptedTos, preferredUsername }) };
} catch (e) {
    if(e instanceof Error) throw e;
    else return { status: "error", error: e  as any };
}
...
export type LauncherUrls = { server_api: string; hub_api: string | null; auth_base: string | null; steam_auth: string | null; byond_hash_api: string | null; register_url: string | null; account_url: string | null; help_url: string }
...
export type SteamAuthResult = { success: boolean; user_exists: boolean; access_token: string | null; requires_linking: boolean; linking_url: string | null; error: string | null; requires_tos: boolean; suggested_username: string | null }
